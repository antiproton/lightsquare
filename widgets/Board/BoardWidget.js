...
function BoardWidget(parent) {
	...
	*/
	...
	this.MoveMode=BoardWidget.MOVE_MODE_CLICK_CLICK|BoardWidget.MOVE_MODE_DRAG_DROP;
	this.MoveInfo={};
	this.ResetMoveInfo();
...
BoardWidget.SQ_ZINDEX_ABOVE=5; //currently dragging square
BoardWidget.SQ_ZINDEX_NORMAL=4; //normal square
BoardWidget.SQ_ZINDEX_BELOW=2; //square highlight nodes

//exponential enum constants

BoardWidget.MOVE_MODE_NONE=1;
BoardWidget.MOVE_MODE_CLICK_CLICK=2;
BoardWidget.MOVE_MODE_DRAG_DROP=4;

/*
initialise default square highlighting styles
*/

BoardWidget.prototype.init_hilite_styles=function() {
	/*
...
BoardWidget.prototype.init_props=function() {
...
BoardWidget.prototype.SetupHtml=function() {
	var self=this;
	...
	*/
	...
	for(var r=0; r<8; r++) {
		for(var f=0; f<8; f++) {
			...
			style(sq_inner, {
				position: "absolute",
				zIndex: BoardWidget.SQ_ZINDEX_NORMAL
			});

			style(highlight, {
				position: "absolute",
				zIndex: BoardWidget.SQ_ZINDEX_BELOW,
				borderStyle: "solid",
	...
	*/
	...
	this.PromoteDialog.Zindex(BoardWidget.SQ_ZINDEX_ABOVE);
	...
	*/
	...
	this.GameOverDialog.Zindex(BoardWidget.SQ_ZINDEX_ABOVE);
	...
	*/
	...
	this.ForceResignDialog.Zindex(BoardWidget.SQ_ZINDEX_ABOVE);
...
BoardWidget.prototype.UpdateHtml=function() { //after switching colours ,changing size tec
...
BoardWidget.prototype.SetSquare=function(sq, pc) {
...
BoardWidget.prototype.SetHtmlSquare=function(sq, pc) {
	var bgimg="none";
...
BoardWidget.prototype.UpdateSquares=function() {
	for(var sq=0; sq<this.Board.length; sq++) {
		this.SetHtmlSquare(sq, this.Board[sq]);
	}
}

BoardWidget.prototype.ResetMoveInfo=function() {
	this.MoveInfo={
		Mode: BoardWidget.MOVE_MODE_CLICK_CLICK,
		Selected: false,
		InProgress: false,
		Piece: null,
		From: null,
...
BoardWidget.prototype.SqFromMouseEvent=function(e, use_offsets, offsets) { //useoffsets to calc for middle of piece
...
BoardWidget.prototype.sq_from_offsets=function(x, y) {
...
BoardWidget.prototype.SetSquarePos=function(square, sq) {
	var x, y;
	var r=Util.y(sq);
	var f=Util.x(sq);
...
BoardWidget.prototype.ResetSquarePos=function(square) { //return the inner bit to its container pos
	style(square.Node, {
		top: 0,
		left: 0
	});
...
BoardWidget.prototype.SetSquareXyPos=function(square, x, y) { //takes mouse coords
...
BoardWidget.prototype.BoardMouseDown=function(e) {
	e.preventDefault();
	...
	if(this.MouseOnBoard(e)) {
		...
		if(this.MoveMode!==BoardWidget.MOVE_MODE_NONE && !this.MoveInfo.Selected && !this.MoveInfo.InProgress && this.Board[sq]!==SQ_EMPTY) { //first click or start of drag
...
BoardWidget.prototype.BoardMouseMove=function(e) {
	e.preventDefault();
	...
	if(this.MoveInfo.Selected && !this.MoveInfo.InProgress) { //down and not already up on same square
		...
		if(args.Cancel) {
			...
			this.MoveInfo.Mode=BoardWidget.MOVE_MODE_DRAG_DROP;
			this.MoveInfo.InProgress=true;
			this.PieceSelected.Fire({Sq: sq});
		}
	}

	if(this.MoveInfo.Selected && this.MoveInfo.Mode===BoardWidget.MOVE_MODE_DRAG_DROP) {
		args={
			Square: sq,
...
BoardWidget.prototype.BoardMouseUp=function(e) {
	e.preventDefault();
	...
	if(this.MoveInfo.InProgress && this.MoveInfo.Mode===BoardWidget.MOVE_MODE_DRAG_DROP) {
	...
	if(this.MoveInfo.Mode===BoardWidget.MOVE_MODE_CLICK_CLICK) {
		this.SquareClicked.Fire(args);
	}

	else if(this.MoveInfo.Mode===BoardWidget.MOVE_MODE_DRAG_DROP && this.MoveInfo.InProgress) {
	...
	if(!args.Cancel) {
		if(this.MoveInfo.InProgress) { //was dragging, now dropped; or second click
		...
		else if(this.MoveMode&BoardWidget.MOVE_MODE_CLICK_CLICK && this.MoveInfo.Selected && sq===this.MoveInfo.From && !this.MoveInfo.InProgress) { //clicking on first square
			args={
				Square: sq,
				Piece: this.Board[sq],
				Dragging: false,
			...
			if(args.Cancel) {
				...
				this.MoveInfo.InProgress=true;
				this.MoveInfo.Mode=BoardWidget.MOVE_MODE_CLICK_CLICK;
				this.PieceSelected.Fire({Sq: sq});
			}
		}
	}
...
BoardWidget.prototype.inc_z_index=function(square) {
	style(square.Node, {
		zIndex: BoardWidget.SQ_ZINDEX_ABOVE
	});
}

BoardWidget.prototype.reset_z_index=function(square) {
	style(square.Node, {
		zIndex: BoardWidget.SQ_ZINDEX_NORMAL
	});
}

BoardWidget.prototype.MouseOnBoard=function(e, use_offsets, offsets) {
	offsets=offsets||[this.MoveInfo.OffsetX, this.MoveInfo.OffsetY];
...
BoardWidget.prototype.coords_on_board=function(x, y) {
...
BoardWidget.prototype.HiliteSq=function(sq, style) {
	style(this.Squares[sq].Highlight, style);
}

BoardWidget.prototype.UnhiliteSq=function(sq) {
	if(sq!==null) {
		style(this.Squares[sq].Highlight, this.HlNone);
	}
}

BoardWidget.prototype.HilitePossibilities=function(sqs) {
...
BoardWidget.prototype.UnhiliteLastMoveFrom=function() {
	this.UnhiliteSq(this.HilitLastMoveFrom);
	this.HilitLastMoveFrom=null;
}

BoardWidget.prototype.UnhiliteLastMoveTo=function() {
	this.UnhiliteSq(this.HilitLastMoveTo);
	this.HilitLastMoveTo=null;
}

BoardWidget.prototype.UnhiliteCanSelect=function() {
	this.UnhiliteSq(this.HilitCanSelect);
	this.HilitCanSelect=null;
}

BoardWidget.prototype.UnhiliteCanDrop=function() {
	this.UnhiliteSq(this.HilitCanDrop);
	this.HilitCanDrop=null;
}

BoardWidget.prototype.UnhiliteSelected=function() {
	this.UnhiliteSq(this.HilitSelected);
	this.HilitSelected=null;
}
...
BoardWidget.prototype.UpdateMouseOverData=function(e) {
	var sq=this.SqFromMouseEvent(e);
...
BoardWidget.prototype.UpdatePieceOverData=function(e) {
...
BoardWidget.prototype.Deselect=function() {
...
BoardWidget.prototype.AnimateMove=function(piece, fs, ts, callback) {
	//animate the move
...
BoardWidget.prototype.get_overall_size=function(dimension) {
